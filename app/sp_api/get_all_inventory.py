...
async def get_all_inventory(page_size,  authorization_header : str = Depends(lambda x: x.headers.get("Authorization")),domain_prefix: str = Depends(lambda x: x.headers.get("domain_prefix", ""))):
 try:
    ...
    api_url = f"https://{domain_prefix}.retail.lightspeed.app/api/2.0/inventory?page_size={page_size}"
