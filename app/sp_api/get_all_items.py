...
async def get_all_items(page_size, authorization_header: str = Depends(lambda x: x.headers.get("Authorization")),domain_prefix: str = Depends(lambda x: x.headers.get("domain_prefix", ""))):
    try:
        ...
        api_url = f"https://{domain_prefix}.vendhq.com/api/2.0/products?page_size={page_size}"
